...
<body class="bg-cover bg-gray-900 text-white min-h-screen flex flex-col justify-center items-center">
    ...
    <section class="content py-24 relative">
        <div class="w-full max-w-7xl mx-auto px-4 md:px-8">
            <h2 class="font-manrope font-extrabold text-3xl leading-10 text-white mb-9">Order History</h2>
            ...
            <c:choose>
                    ...
                    <c:forEach var="item" items="${order}">
                        <div class="mt-7 border border-gray-300 pt-9 box rounded-3xl bg-gray-700 grid cursor-pointer transition-all duration-500">
                            <div class="flex max-md:flex-col items-center justify-between px-3 md:px-11">
                                <div class="data">
                                    <p class="font-medium text-lg leading-8 text-white whitespace-nowrap">Order : #${item.orderid}</p>
                                </div>
                                <div>
                                    <p class="font-medium text-lg leading-8 text-white mt-3 whitespace-nowrap">Order Date : ${item.orderDate.dayOfMonth}-${item.orderDate.month}-${item.orderDate.year}</p>
                                </div>
                            </div>
                            <c:choose>
                                        ...
                                        <div class="flex max-lg:flex-col items-center gap-8 lg:gap-24 px-3 md:px-11">
                                            <div class="grid grid-cols-4 w-full">
                                                <div class="col-span-4 sm:col-span-1">
                                                    <img src="data:image/png;base64,${orderdata.food.base64}" alt="${orderdata.food.foodname}" class="max-sm:mx-auto object-cover">
                                                </div>
                                                <div class="col-span-4 sm:col-span-3 max-sm:mt-4 sm:pl-8 flex flex-col justify-center max-sm:items-center">
                                                    <h6 class="font-manrope font-semibold text-2xl leading-9 text-white mb-3 whitespace-nowrap">${orderdata.food.foodname}</h6>
                                                    <p class="font-normal text-lg leading-8 text-gray-300 mb-8 whitespace-nowrap">By: Online Food Ordering</p>
                                                    <div class="flex items-center max-sm:flex-col gap-x-10 gap-y-3">
                                                        <span class="font-normal text-lg leading-8 text-gray-300 whitespace-nowrap">Qty: ${orderdata.quantity}</span>
                                                        <span class="font-semibold text-xl leading-8 text-gray-300 whitespace-nowrap">Price: <span style="color: white;">&#x20B9; ${orderdata.price}</span></span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="flex items-center justify-around w-full sm:pl-28 lg:pl-0">
                                                <div class="flex flex-col justify-center items-start max-sm:items-center">
                                                    <p class="font-normal text-lg text-gray-300 leading-8 mb-2 text-left whitespace-nowrap">Status</p>
                                                    <p class="font-semibold text-lg leading-8 text-left whitespace-nowrap" style="color: ${item.status == 'Cancelled' || item.status == 'cancelled' ? 'red' : 'green'};">${item.status}</p>
                                                </div>
                                                <div class="flex flex-col justify-center items-start max-sm:items-center">
                                                    <p class="font-normal text-lg text-gray-300 leading-8 mb-2 text-left whitespace-nowrap">Ordered By : ${item.customer.firstname}</p>
                                                    <p class="font-semibold text-lg leading-8 text-white text-left whitespace-nowrap">Delivery Address : ${item.deliveryaddress}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </c:forEach>
                            ...
                            <div class="px-3 md:px-11 flex items-center justify-between max-sm:flex-col-reverse">
                                ...
                                <p class="font-medium text-xl leading-8 text-black max-sm:py-4">
                                    <span class="text-gray-300">Total Price: </span> &nbsp;<span style="color: white;"><span>&#x20B9;</span>${item.finalprice}</span>
                                </p>
                            </div>
                        </div>
                    </c:forEach>
                ...
                <c:otherwise>
                    <div class="text-center py-10">
                        <h1 class="text-red-500 text-2xl font-semibold">No Order Found</h1>
                    </div>
                </c:otherwise>
            </c:choose>
        </div>
